...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    ee_target_set = torch.cat(
        [ee_goal_pose_set_tilted_b, ee_goal_wrench_set_tilted_task, kp_set_task], dim=-1
    )
    ...
    while simulation_app.is_running():
        ...
        if count % 500 == 0:
            ...
            robot.set_joint_effort_target(
                zero_joint_efforts
            )  # Set zero torques in the initial step
            ...
            command, ee_target_pose_b, ee_target_pose_w, current_goal_idx = (
                update_target(
...
def update_states(
    ...
    ee_lin_vel_b = quat_rotate_inverse(
        robot.data.root_quat_w, relative_vel_w[:, 0:3]
    ...
    ee_force_w, _ = torch.max(
        torch.mean(contact_forces.data.net_forces_w_history, dim=1), dim=1
    )
